...
def _score_next_frame_pixels(
    ...
    pixel_pred_pca = cross_val_predict(
        _make_mlp(), pixel_input_pca, target_pixel_pca, cv=5
    )
    ss_res = np.sum((target_pixel_pca - pixel_pred_pca) ** 2)
    ss_tot = np.sum(
        (target_pixel_pca - target_pixel_pca.mean(axis=0, keepdims=True)) ** 2
    )
    ...
    physics_r2 = _pixel_prediction_r2(
    ...
    if target_raw is not None and initial_raw is not None:
        ...
        ss_tot_o = np.sum(
            (delta_target[:n] - delta_target[:n].mean(axis=0, keepdims=True)) ** 2
        )
...
def _compute_predicted_frames(
    ...
    def pca_to_img_upscaled(pred_pca):
        ...
        raw = (
            ...
            .astype(np.uint8)
            .reshape(n, IMAGE_SIZE, IMAGE_SIZE, 4)
        )
        upscaled = np.stack(
            [
                np.array(
                    _Image.fromarray(raw[i]).resize(
...
def _score_kinetic_energy(pixel_pca, physics_scaled, behavior_labels):
    ...
    pixel_r2 = cross_val_score(
        log_reg, pixel_pca, behavior_labels, cv=5, scoring="accuracy"
    ).mean()
    physics_r2 = cross_val_score(
        log_reg, physics_scaled, behavior_labels, cv=5, scoring="accuracy"
    ).mean()
    ...
    *,
    ...
    if objective == "next_frame_pixels":
        ...
        combined_score = cross_val_score(
            log_reg, combined_features, behavior_labels, cv=5, scoring="accuracy"
        ).mean()
    ...
    if predicted_pixel_pca is not None:
        ...
        if objective == "next_frame_pixels":
            ...
            predicted_pixel_score = cross_val_score(
                log_reg, predicted_input_pca, behavior_labels, cv=5, scoring="accuracy"
            ).mean()
    ...
    if forward_program_states is not None:
        ...
        fwd_late_rgba = forward_program_states[
            :n_fwd, fri["late"].start : fri["late"].start + rgba_n
        ]
        from config import IMAGE_SIZE as _IMG

        fwd_imgs = (
            np.clip(fwd_late_rgba, 0, 255)
            .astype(np.uint8)
            .reshape(n_fwd, _IMG, _IMG, 4)
        )
    ...
    print("\n  DISSOCIATION:")
    print(
        f"    Pixel model:          R² = {mean_r2_pixel:.4f}  |  {metric_label} = {pixel_score:.4f}"
    )
    print(
        f"    Physics model:        R² = {mean_r2_physics:.4f}  |  {metric_label} = {physics_score:.4f}"
    )
    print(
        f"    Pixel+physics model:  R² = {mean_r2_combined:.4f}  |  {metric_label} = {combined_score:.4f}"
    )
    if predicted_pixel_score is not None:
        print(
            f"    Predicted-S model:    R² = {r2_predicted_pixel_dissoc.mean():.4f}  |  {metric_label} = {predicted_pixel_score:.4f}"
        )
    ...
    result = {
        ...
        "fg_physics_behavioral_score": (
            float(fg_physics_score) if fg_physics_score is not None else float("nan")
        ),
        "delta_pixel_behavioral_score": (
            float(delta_pixel_score) if delta_pixel_score is not None else float("nan")
        ),
        "delta_physics_behavioral_score": (
            float(delta_physics_score)
            if delta_physics_score is not None
            else float("nan")
        ),
