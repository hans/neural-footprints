...
def _score_next_frame_pixels(render_input_pca, target_pixel_pca,
    ...
    Render model score:  MLP cross-val R² predicting target-frame pixel PCA
                         from the brain's 3-frame render PCA. Even with three
    ...
    # Render model: MLP cross-val R² in target PCA space
    render_r2 = cross_val_score(_make_mlp(), render_input_pca, target_pixel_pca,
...
def _compute_predicted_frames(
    render_input_pca, target_pixel_pca, scaler_target, pca_target,
    ...
    Returns (init_imgs, render_imgs, physics_imgs, target_imgs) as uint8 arrays.
    ...
    # Render model: MLP trained on 3-frame render PCA → target RGBA PCA
    render_model = _make_mlp()
    render_model.fit(render_input_pca, target_pixel_pca)
    render_pred_pca = render_model.predict(render_input_pca[:n])
    ...
    return init_imgs, render_imgs, physics_imgs, target_imgs
...
def run_dissociation_analysis(neural_activity, scenes, neural_meta,
    ...
    if objective == "next_frame_pixels":
        render_score, physics_score, metric_label, chance = _score_next_frame_pixels(
            render_input_pca, target_pixel_pca,
            scene_configs, initial_physics_labels,
