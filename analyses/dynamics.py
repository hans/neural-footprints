...
def run_dynamics_analysis(neural_activity, scenes, neural_meta,
    ...
    if inferred_physics is not None:
        ...
        resim_inferred_render = resim_inferred_states[:, render_indices]

        print("  Cross-validating encoder on inferred-resimulated renders (5-fold)...")
        ...
        for fold, (train_idx, test_idx) in enumerate(kf.split(render_data), 1):
            ...
            pipe.fit(render_data[train_idx], neural_activity[train_idx])
            pred_neural_inferred[test_idx] = pipe.predict(resim_inferred_render[test_idx])
