...
def build_frame_stack_with_depth(scenes):
    ...
    # Clip at 10 m before computing range: sky pixels (>100 m with wider FOVs) would
    # otherwise dominate d_max and compress all foreground to near-zero after scaling.
    all_depth = np.concatenate([d_init, d_mid, d_late], axis=0)
    all_depth = np.clip(all_depth, None, 10.0)
    d_min = all_depth.min()
    d_max = all_depth.max()
    d_range = max(d_max - d_min, 1e-6)

    def norm_depth(d_raw):
        return ((np.clip(d_raw, None, 10.0) - d_min) / d_range).reshape(N, 1, H, W)
...
def main():
    ...
    if unknown:
        ap.error(
            f"unknown configs: {sorted(unknown)}; available: {sorted(SWEEP_CONFIGS)}"
        )
    ...
    if render_per_frame == expected_rgba_only:
        print(
            f"  WARNING: scenes appear to be old PyBullet RGBA-only format "
            f"(render_per_frame={render_per_frame}). Depth-aug will not work correctly.",
            flush=True,
        )
    elif render_per_frame == expected_full:
        print(
            f"  scenes: MuJoCo format with depth+seg (render_per_frame={render_per_frame})",
            flush=True,
        )
    else:
        print(
            f"  scenes: render_per_frame={render_per_frame} "
            f"(expected {expected_full} for full MuJoCo)",
            flush=True,
        )
    ...
    if has_depth:
        ...
        print(
            f"  depth shape={frames_depth.shape}  dtype={frames_depth.dtype}",
            flush=True,
        )
        # Quick sanity: depth channel should be in [0, 1]
        depth_vals = frames_depth[:, :, 4, :, :]
        print(
            f"  depth stats: min={depth_vals.min():.4f}  max={depth_vals.max():.4f}  "
            f"mean={depth_vals.mean():.4f}",
            flush=True,
        )
    else:
        frames_depth = None
    ...
    print(
        f"  valid dims: {[PHYSICS_LABELS[i] for i in range(full_physics_dim) if valid_dims[i]]}",
        flush=True,
    )
    ...
    if args.val_frac <= 0.0:
        ...
        idx_tr, idx_val = train_test_split(
            idx, test_size=args.val_frac, random_state=42
        )
    ...
    for name in args.configs:
        ...
        print(
            f"  model_cls={model_cls_name}  n_channels={n_channels}  kwargs={kwargs}",
            flush=True,
        )
        ...
        if model_cls_name not in _MODEL_REGISTRY:
            raise ValueError(
                f"unknown model_cls: {model_cls_name}; "
        ...
        for i, lbl in enumerate(PHYSICS_LABELS):
            if valid_dims[i]:
                print(
                    f"     {i:2d} {lbl:10s}  R²={full_per_dim_r2[i]:+.4f}", flush=True
                )
    ...
    print("\n=== Summary ===", flush=True)
    print(
        f"  {'config':20s}  {'model_cls':26s}  {'params':>10s}  "
        f"{'mean R²':>8s}  {'min R²':>8s}  {'epoch':>6s}",
        flush=True,
    )
    for name, r in results.items():
        if r.get("skipped"):
            ...
            continue
        print(
            f"  {name:20s}  {r['model_cls']:26s}  {r['n_params']:>10,}  "
