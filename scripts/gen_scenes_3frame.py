...
from scene_generator import (
    _create_scene,
    _collect_physics_labels,
    _sample_lighting,
    _encode_scene_config,
    _encode_scene_lighting,
    _compute_total_kinetic_energy,
...
def generate_3frame_scenes(n_scenes, seed, n_timesteps, t_mid, t_late, fov):
    ...
    for i in range(n_scenes):
        ...
        body_ids, masses, frictions, _, shape_configs, pillar_gray = _create_scene(
            pc, scene_rng
        )
        ...
        for t in range(n_timesteps):
            ...
            if t + 1 == t_mid:
                ...
                mid_renders[i] = np.frombuffer(mid_rgba, dtype=np.uint8).astype(
                    np.float32
                )
            if t + 1 == t_late:
                late_rgba, _, _ = _render_with_fov(pc, lighting, fov)
                late_renders[i] = np.frombuffer(late_rgba, dtype=np.uint8).astype(
                    np.float32
                )
...
def main():
    ...
    print(
        f"3-frame generator: t_mid={args.t_mid}, t_late={args.t_late}, "
