...
class DownsampleMLP(nn.Module):
    ...
    def __init__(
        self, n_frames, n_channels, image_size, output_dim, pool=8, hidden_dim=256
    ):
        super().__init__()
        ds = image_size // pool
...
class GridPoolCNN(nn.Module):
    ...
    def __init__(self, n_frames, n_channels, output_dim, grid_size=4, hidden_dim=256):
        super().__init__()
...
class SpatialSoftmaxCNN(nn.Module):
    ...
    def __init__(
        self, n_frames, n_channels, image_size, output_dim, n_filters=32, hidden_dim=64
    ):
        super().__init__()
...
def train_one(
    ...
    *,
    ...
    print(
        f"  [{label}] training: device={device}  n_train={X_tr.shape[0]}  n_val={X_val.shape[0]}",
        flush=True,
    )
    ...
    for epoch in range(n_epochs):
        ...
        if (epoch + 1) % log_every == 0 or epoch == 0:
            print(
                f"  [{label}]  ep {epoch+1:>3d}  train={tl:.4f}  val={vl:.4f}  lr={cur_lr:.2e}",
                flush=True,
            )
        sch.step(vl)
        ...
        if vl < best_val - 1e-5:
            ...
            if bad_epochs >= patience and (epoch + 1) >= min_epochs:
                print(
                    f"  [{label}] early stop @ epoch {epoch+1}  best val={best_val:.4f} @ {best_epoch}",
                    flush=True,
                )
                break
    ...
    print(
        f"  [{label}] done in {elapsed:.1f}s  best val={best_val:.4f} @ epoch {best_epoch}",
        flush=True,
    )
...
def main():
    ...
    for variant in variants:
        ...
        for i, name in enumerate(PHYSICS_LABELS):
            if valid_dims[i]:
                print(
                    f"     {i:2d} {name:10s}  R²={full_per_dim_r2[i]:+.4f}", flush=True
                )
    ...
    print("\n=== Summary ===", flush=True)
    print(
        f"  {'variant':20s}  {'params':>10s}  {'val_mse':>8s}  {'mean R²':>8s}  {'min R²':>8s}  {'epoch':>6s}  {'time(s)':>8s}",
        flush=True,
    )
    for v, r in results.items():
        print(
            f"  {v:20s}  {r['n_params']:>10,}  {r['val_mse']:>8.4f}  "
            f"{r['mean_valid_dim_r2']:>+8.4f}  {r['min_valid_dim_r2']:>+8.4f}  "
            f"{r['best_epoch']:>6d}  {r['train_time_s']:>8.1f}",
            flush=True,
        )
