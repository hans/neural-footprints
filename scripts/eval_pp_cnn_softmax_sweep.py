...
def main():
    ...
    if unknown:
        ap.error(
            f"unknown configs: {sorted(unknown)}; "
    ...
    print("\n=== Summary ===", flush=True)
    print(
        f"  {'config':24s}  {'params':>10s}  {'val_mse':>8s}  {'mean R²':>8s}  "
        f"{'min R²':>8s}  {'epoch':>6s}  {'time(s)':>8s}",
        flush=True,
    )
    for name, r in results.items():
        print(
            f"  {name:24s}  {r['n_params']:>10,}  {r['val_mse']:>8.4f}  "
