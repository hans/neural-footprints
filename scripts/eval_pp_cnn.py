...
def _encoding_metrics(
    ...
    effective_render_pca_dim = min(
        render_pca_dim, render_data.shape[0] - 1, render_data.shape[1]
    )
    render_pca, _, _ = pca_reduce_render(
        render_data, effective_render_pca_dim, random_state=random_state
    )
...
def main():
    ...
    print(
        f"  trained in {train_time:.1f}s  best val MSE={best_val_mse:.4f} "
    ...
    if not args.no_encoding:
        ...
        print(
            f"  inferred_physics={inferred_physics.shape}  "
        ...
        t0 = time.time()
        enc = _encoding_metrics(
            neural, scenes, inferred_physics, render_pca_dim=cfg["render_pca_dim"]
        )
    ...
    target_per_dim = 0.40
    decision = (
        "PASS" if (mean_r2 >= target_mean and min_r2 >= target_per_dim) else "FAIL"
    )
    metrics["decision"] = decision
    metrics["decision_thresholds"] = {
        "mean_r2": target_mean,
        "min_dim_r2": target_per_dim,
    }
