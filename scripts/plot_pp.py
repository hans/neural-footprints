...
rgba_n = (metadata['target_pixel_indices'].stop
          - metadata['target_pixel_indices'].start)
fwd_init_rgba = fwd_states[frame_idx, fri['initial'].start:fri['initial'].start + rgba_n]
fwd_frame_imgs = np.clip(fwd_init_rgba, 0, 255).astype(np.uint8).reshape(
    n_pp, IMAGE_SIZE, IMAGE_SIZE, 4)
