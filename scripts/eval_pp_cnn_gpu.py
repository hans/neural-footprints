...
def ensure_scenes(scenes_path, n_scenes_override=None):
    ...
    print(f"{scenes_path} not found — generating fixture (no Snakemake) ...", flush=True)
    cfg = load_config()
...
def train_cnn(net, frames_f32, y, idx_tr, idx_val, *, device,
              n_epochs, batch_size, lr, patience, log_every):
    ...
    print(f"  training: device={device}  n_train={X_tr.shape[0]}  n_val={X_val.shape[0]}", flush=True)
    ...
    for epoch in range(n_epochs):
        ...
        if (epoch + 1) % log_every == 0 or epoch == 0:
            print(f"  {epoch+1:>5d}  {tl:>10.4f}  {vl:>10.4f}  {cur_lr:>8.2e}", flush=True)
        sch.step(vl)
        ...
        if vl < best_val - 1e-5:
            ...
            if bad_epochs >= patience:
                print(f"  early stop @ epoch {epoch+1}, "
                      f"best val={best_val:.4f} @ epoch {best_epoch}", flush=True)
                break
...
def encoding_metrics(neural, scenes, inferred_physics, render_pca_dim,
    ...
    render_pca, _, _ = pca_reduce_render(render_data, eff_dim, random_state=random_state)
...
def main():
    ...
    print(f"\nBuilding net (BN={use_bn}, hidden_dim={args.hidden_dim}, "
          f"output_dim={n_observable}) ...", flush=True)
    net = InverseCNNNet(
        n_frames=3, n_channels=4, output_dim=n_observable,
        hidden_dim=args.hidden_dim, dropout_rate=args.dropout, batch_norm=use_bn,
    )
    n_params = sum(p.numel() for p in net.parameters())
    print(f"  params: {n_params:,}", flush=True)

    fit_out = train_cnn(net, frames_f32, y, idx_tr, idx_val, device=device,
    ...
    print(f"  mean over valid dims: {mean_r2:+.4f}    min dim: {min_r2:+.4f}", flush=True)
    ...
    metrics = {
        ...
        'valid_dims': [PHYSICS_LABELS[i] for i in range(full_physics_dim) if valid_dims[i]],
    ...
    if not args.no_encoding:
        ...
        print(f"  inferred_physics={inferred_physics.shape}  h2={h2.shape}  "
              f"in {time.time()-t0:.1f}s", flush=True)

        render = scenes['program_states'][:, scenes['metadata']['render_indices']]
        neural_input = np.concatenate([render, h2, inferred_physics], axis=1).astype(np.float32)
    ...
    if args.curves:
        ...
        plot_curves(fit_out['history'], args.curves,
                    best_epoch=fit_out['best_epoch'], lr=args.lr)
        print(f"Saved learning curves → {args.curves}", flush=True)

    print("\n=== Decision (specs/inverse_model_input_repr.md success criterion) ===",
          flush=True)
    print(f"  mean R²    = {mean_r2:+.4f}  vs target {target_mean:+.2f}  "
          f"{'PASS' if mean_r2 >= target_mean else 'FAIL'}", flush=True)
    print(f"  min dim R² = {min_r2:+.4f}  vs target {target_min:+.2f}  "
          f"{'PASS' if min_r2 >= target_min else 'FAIL'}", flush=True)
