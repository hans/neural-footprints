...
REFERENCE_ROWS = [
    ...
    ('gridpool (simple)',     1700197, 0.7963, {
        'pos_x': 0.9813, 'pos_y': 0.8715, 'pos_z': 0.6375,
        'linvel_x': 0.8410, 'x_accel': 0.6502,
    }),
    ('spatial_softmax v1',    34405,   0.7564, {
        'pos_x': 0.9764, 'pos_y': 0.8107, 'pos_z': 0.5717,
        'linvel_x': 0.8418, 'x_accel': 0.5812,
    }),
]
...
def main():
    ...
    for name, r in d['results'].items():
        ...
        rows.append((name, r['n_params'], r['mean_valid_dim_r2'],
                     {k: per_dim.get(k) for k in PER_DIM_COLS}))

    # Header
    head = (f"  {'config':24s}  {'params':>10s}  {'mean R²':>8s}  "
            + "  ".join(f"{c:>8s}" for c in PER_DIM_COLS))
    print(head)
    print('  ' + '-' * (len(head) - 2))
    ...
    for label, n_params, mean_r2, per_dim in rows:
        ...
        print(f"  {label:24s}  {fmt_int(n_params):>10s}  {fmt_r2(mean_r2):>8s}  {cells}")
    ...
    print(f"  vs gridpool:  perf = {best[1]['mean_valid_dim_r2']/gp_mean*100:.1f}%  "
